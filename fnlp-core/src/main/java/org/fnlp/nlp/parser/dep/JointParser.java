...
public class JointParser implements Serializable {
    ...
    private void doNext(String action, JointParsingState state) {
        ...
        switch (act) {
            case 'L':
                state.next(JointParsingState.Action.LEFT, relation);
                break;
            case 'R':
                state.next(JointParsingState.Action.RIGHT, relation);
                break;
            default:
    ...
    private void doNext(String action, float est1, JointParsingState state) {
        ...
        switch (act) {
            case 'L':
                state.next(JointParsingState.Action.LEFT, est1, relation);
                break;
            case 'R':
                state.next(JointParsingState.Action.RIGHT, est1, relation);
                break;
            default:
                System.out.println("状态动作错误");
        }
